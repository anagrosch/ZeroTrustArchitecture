...
from trust_signal_collection import get_keycloak_events
        ...
        if oidc.user_loggedin:
            ...
            if token_is_valid(oidc,keycloak_openid):
                if 'oidc_auth_profile' in session:
                    ...
                    cleaned_data = get_keycloak_events(keycloak_admin)

                    keycloak_events = {
                        'cleaned_data': cleaned_data,
                        'intent': 'store_keycloak_events'
                    }
                    ...
                    # Send data to Data Center through Access Proxy
                    node4.send_message_to_node('1', keycloak_events)
                    ...
                    # Send new user data to data center
                    user_update = {
                        'extracted_data': extracted_data,
                        'intent': 'update_users'
                    }
                    node4.send_message_to_node('1', user_update)
                    node4.stop()
...
def update_local_file(access_request):
    ...
    file_path = os.path.join(os.getcwd(), 'access_requests.json')
    existing_data = []
    new_id = 1
    ...
    # Add ID to access request
    id = {'ID': new_id}
    access_request = {**id, **access_request}
...
def receive_policy_configurations():
    if request.method == 'POST':
        ...
        if status == 'success':
            ...
            return 'Received Policy Configurations failed!', 500
    else:
...
@app.route('/logging')
def access_requests():
    file_path = os.path.join(os.getcwd(), 'access_requests.json') #refers to local file copy
...
def hidden_resource():
    ...
    secret_message = RESOURCE_SECRET_KEY # assign it to the randomized secret generated at the PAM endpoint
