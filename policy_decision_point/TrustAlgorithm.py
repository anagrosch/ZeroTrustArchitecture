...
def calculate_authentication_data_score(authentication_data):
    ...
    elif 0.7 <= sign_in_risk < 0.9:
        sign_in_risk_score = 0.75
    elif 0.5 <= sign_in_risk < 0.7:
...
def calculate_access_request_score(access_request_data, night_start, night_end, high_risk_locations, medium_risk_locations, low_risk_locations):
    location_score = 0.0
   ...
   # Evaluate location, access request time, device_os, and device_type
    ...
    # Check if the access time falls within the specified nighttime boundaries
...
def calculate_overall_trust_score(user_data, ta_data):
    # Get user data using provided functions
    identity_data = user_data.get('user')
    access_request_data = user_data.get('latest_request')
    authentication_data = user_data.get('latest_data')

    # Get policy configurations
    high_risk_countries = ta_data.get('high_risk_countries')
    medium_risk_countries = ta_data.get('medium_risk_countries')
    low_risk_countries = ta_data.get('low_risk_countries')
    period_start = ta_data.get('period_start')
    period_end = ta_data.get('period_end')
    ...
    access_request_score = calculate_access_request_score(access_request_data,
                                                          ...
                                                          high_risk_locations=high_risk_countries,
                                                          medium_risk_locations=medium_risk_countries,
                                                          low_risk_locations=low_risk_countries)
