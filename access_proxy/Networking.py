...
class Networking(Node):
    ...
    def process_message_from_web_ui(self, sender, message):
        ...
        if message.get('intent', '').lower() == 'access request':
            ...
            # Request trust score from Trust Engine (node 2)
            self.send_message_to_node('2', data)

            # Forward message to Data Center (node 5) to add request to JSON file
            self.send_message_to_node('5', message)
        ...
        # Store Keycloak events to events.json
            ...
            # Prepare data to send to Data Center (node 5)
    ...
    def node_message(self, node, data):
        ...
        if self.message_is_from_web_ui(sender_id):
